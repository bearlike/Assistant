...
Meeseeks has two editing backends. Both apply edits atomically and return a unified diff so you can see exactly what changed. In the web console, every file edit is rendered as an expandable diff card in the session timeline, with line-level additions and deletions highlighted in green and red.

<div style="display: flex; justify-content: center;">
  <img src="../meeseeks-console-04-file-edit.jpg" alt="A file-edit tool card in the Meeseeks console showing a unified diff with +23 additions and -2 deletions" style="width: 100%; max-width: 720px; height: auto;" />
</div>
...
When `agent.edit_tool` is empty (the default), Meeseeks picks the right backend for the active model automatically. Override it only when you want to force a single backend regardless of which model is running.
