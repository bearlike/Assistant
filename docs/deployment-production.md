...
Running Meeseeks in production means hardening the default Docker Compose setup with TLS termination, proper authentication tokens, CORS restriction, and observability. This page covers the production checklist and the nginx reverse proxy configuration.
...
```bash
sudo ln -s /path/to/meeseeks/docker/nginx-reverse-proxy.conf \
           /etc/nginx/sites-enabled/meeseeks
...
server {
    ...
    server_name meeseeks.example.com;
...
### Key nginx settings for Meeseeks
