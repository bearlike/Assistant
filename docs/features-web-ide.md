...
```bash
curl -sk -X POST https://truss.example.com/api/sessions/abc123.../ide \
...
```bash
curl -sk -X POST https://truss.example.com/api/sessions/abc123.../ide/extend \
...
| Reconnect | `POST .../ide` on an existing session returns the current URL and password. If the container exited unexpectedly, Truss respawns it automatically. |
...
| `network` | `truss-ide` | Docker network the containers join. |
| `state_dir` | `/tmp/truss-ide` | Host directory used for bookkeeping files. |
