"""Custom integration to integrate mewbo_conversation with Home Assistant.
...
class MewboMessage(TypedDict, total=False):
    """Message structure used to call the Mewbo API."""
...
class MewboResponse(TypedDict):
    """Response payload returned from the Mewbo API."""
...
def _missing_homeassistant_error() -> RuntimeError:
    error = RuntimeError(
        "Home Assistant is not installed. Install mewbo-ha-conversation[homeassistant]."
    )
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    """Set up Mewbo conversation using UI.
    ...
    Raises:
        ConfigEntryNotReady: If the Mewbo API is unreachable.
    """
    ...
    hass.data[DOMAIN][entry.entry_id] = coordinator = MewboDataUpdateCoordinator(
        hass,
        client,
    )
    ...
    try:
        ...
        if not response:
            raise ApiClientError("Invalid Mewbo server")
    except ApiClientError as err:
        raise ConfigEntryNotReady(err) from err

    entry.async_on_unload(entry.add_update_listener(async_reload_entry))

    conversation.async_set_agent(hass, entry, MewboAgent(hass, entry, client))
    return True


async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    """Unload Mewbo conversation.
...
class MewboAgent(_BaseConversationAgent):
    """Mewbo conversation agent."""
    ...
    def __init__(
        ...
        self.entry = entry
        self.client = client
        self.history: dict[str, MewboMessage] = {}
