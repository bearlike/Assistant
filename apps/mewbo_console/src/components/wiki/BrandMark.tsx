/**
 * Clay-flower brand mark used throughout the wiki surface. Single SVG path
 * traced from the design system's logo-transparent.svg; uses currentColor so
 * the parent's `color` (typically `hsl(var(--primary))`) drives the fill.
 */

interface BrandMarkProps {
  size?: number;
  className?: string;
  /** Slow-spin animation, used on the indexing loader and landing hero. */
  spin?: boolean;
}

export function BrandMark({ size = 22, className = "", spin = false }: BrandMarkProps) {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 480 480"
      aria-hidden="true"
      className={`${spin ? "animate-[wiki-spin_24s_linear_infinite]" : ""} ${className}`}
    >
      <path
        d="M480 240c0-29.1-20.7-55.8-55.2-76.5 9.7-39.1 5.5-72.6-15-93.2-20.7-20.6-54.2-24.8-93.3-15.1C295.8 20.7 269.1 0 240 0s-55.8 20.7-76.5 55.2c-39.1-9.7-72.6-5.5-93.2 15s-24.8 54.2-15.1 93.2C20.7 184.3 0 211 0 240s20.7 55.8 55.2 76.5c-9.7 39.1-5.5 72.6 15 93.2 20.7 20.6 54.2 24.8 93.2 15.1C184.2 459.3 211 480 240 480s55.8-20.7 76.5-55.2c39.1 9.7 72.6 5.5 93.2-15s24.8-54.2 15-93.2C459.4 295.8 480 269 480 240Zm-240 60a60 60 0 1 1 0-120 60 60 0 0 1 0 120Z"
        fill="currentColor"
      />
    </svg>
  );
}
