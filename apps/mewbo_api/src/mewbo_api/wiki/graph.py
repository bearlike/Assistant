...
# Mapping from language name to the C function exported by the shared library.
_LANG_FUNC: dict[str, str] = {
...
class GraphIndex:
    ...
    def parse_file(
        ...
        parser = self._parser_for(lang_name, lang)
    ...
    def persist(self, slug: str, result: GraphParseResult, *, store) -> None:
        """Persist nodes + edges to the store (idempotent upsert)."""
        store.upsert_nodes(slug, result.nodes)
        store.upsert_edges(slug, result.edges)

    # ── helpers ───────────────────────────────────────────────────────────────

    def _lang_for(self, lang_name: str):
    ...
    def _parser_for(self, lang_name: str, lang):
        """Return a fresh ``tree_sitter.Parser`` configured for *lang*."""
        from tree_sitter import Parser

        return Parser(lang)

    def _query_for(self, lang_name: str, lang):
...
def _load_ts_language(lang_name: str):
    ...
    func_name = _LANG_FUNC.get(lang_name, f"tree_sitter_{lang_name}")
