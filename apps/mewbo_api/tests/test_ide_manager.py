"""Unit tests for ``mewbo_api.ide`` (IdeManager + IdeStore)."""
...
def test_ensure_creates_container_and_persists(
    ...
    assert instance.container_name == f"mewbo-ide-{VALID_SID}"
    ...
    assert call["name"] == f"mewbo-ide-{VALID_SID}"
...
def test_stop_removes_everything(
    ...
    assert store.get(VALID_SID) is None
    assert not os.path.exists(deadline_file)
    assert f"mewbo-ide-{VALID_SID}" not in fake_client.containers._by_name
