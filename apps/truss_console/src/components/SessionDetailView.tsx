...
export function SessionDetailView({
  ...
  // Slash commands (e.g. `/compact`) hit a dedicated endpoint and don't flow
  // through useSessionQuery — InputBar surfaces its in-flight state via
  // onCommandRunningChange so we can OR it into the same gates that drive
  // the composer's "Sending…" indicator for regular messages.
  const [commandSubmitting, setCommandSubmitting] = useState(false);
  const effectiveSubmitting = submitting || commandSubmitting;
  ...
  const runStatus: RunStatus | undefined = useMemo(() => {
    if (!running && !effectiveSubmitting) return undefined;
    ...
    return {
      phase: phase ?? (effectiveSubmitting ? "Sending…" : "Running"),
      agents: liveAgents.size,
      tokens: sessionUsage?.root_last_input_tokens,
      lastUserTs,
    };
  }, [events, running, effectiveSubmitting, sessionUsage]);
  ...
  const lastRecoverableFailure = useMemo(() => {
    if (running || effectiveSubmitting) return null;
    ...
    return null;
  }, [events, running, effectiveSubmitting]);
  ...
  const conversationPanel = (
    <>
      ...
      <ConversationTimeline
        ...
        isRunning={running || effectiveSubmitting}
      ...
      <div className="relative z-10">
        <InputBar mode="detail" sessionId={session.session_id} sessionContext={effectiveContext} onSubmit={async (query, newContext, mode, attachments) => {
          ...
          await stop();
          resume();
        }} isRunning={running} isSubmitting={effectiveSubmitting} error={queryError} runStatus={runStatus} onCommandRunningChange={setCommandSubmitting} />
      </div>
    </>
  );
