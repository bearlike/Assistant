...
interface InputBarProps {
  ...
  runStatus?: RunStatus;
  /**
   * Notifies the parent when a slash command is in flight so it can mount
   * the same "Sending…" line indicator that regular user messages show.
   * Without this, command sends look silent — the input clears, the
   * network call takes seconds, and the UI gives no acknowledgement.
   */
...
export function InputBar({
  ...
  // useMutation gives us the same in-flight semantics as the regular
  // message path (`useSessionQuery.sendM`) — `isPending` flips true on
  // dispatch and false on settle, and TanStack Query handles error
  // propagation. We forward `isPending` to the parent (see effect below)
  // so SessionDetailView can show the standard "Sending…" indicator.
  const commandMut = useMutation({
    ...
    onSuccess: (result) => {
      ...
      // 'transcript': events stream via the existing event poll/SSE — the
      // chat bubble + LogsView card both arrive through the events feed.
    },
    onError: (err) => {
      setCommandDialog({
  ...
  // Forward in-flight transitions to the parent. SessionDetailView ORs this
  // into the `submitting` flag so the composer's RunIndicator strip mounts
  // and runStatus.phase resolves to "Sending…" — matching the user-message
  // experience exactly. Effect (not onMutate/onSettled) so React batches
  // state updates and we can't desync if the mutation fires twice rapidly.
  useEffect(() => {
    onCommandRunningChange?.(commandMut.isPending);
  }, [commandMut.isPending, onCommandRunningChange]);
