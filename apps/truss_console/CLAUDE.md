<!-- truss:noload -->
# Truss Console — Frontend Engineering Guide

## What this is
A lean React + Vite + Tailwind web console that wraps the Truss API. Not a general-purpose SPA — it's an instrument for watching an agent work. Two principles run in parallel: **library-first** for engineering and **minimal-and-purposeful** for design (sections below). Neither overrides the other.
...
The production console runs at `https://truss.hurricane.home` (self-signed cert).

- **API credentials live in `/home/kk/Projects/Personal-Assistant/docker.env`** — NOT in `app.json` or `~/.truss/app.json`.
...
- **Curl pattern:** Always use `-sk` (silent + skip cert verification) for the self-signed cert:
  ```bash
  curl -sk "https://truss.hurricane.home/api/sessions/<ID>/events" \
