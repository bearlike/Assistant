"""Unit tests for ``truss_api.ide`` (IdeManager + IdeStore)."""
...
from truss_api.ide import (
    DockerUnavailable,
    IdeInstance,
    IdeManager,
    MaxLifetimeReached,
)
from truss_core.config import WebIdeConfig
...
def test_ensure_creates_container_and_persists(
    ...
    assert instance.container_name == f"truss-ide-{VALID_SID}"
    ...
    assert call["name"] == f"truss-ide-{VALID_SID}"
    ...
    assert vols[deadline_file] == {"bind": "/truss/deadline", "mode": "ro"}
...
def test_stop_removes_everything(
    ...
    assert store.get(VALID_SID) is None
    assert not os.path.exists(deadline_file)
    assert f"truss-ide-{VALID_SID}" not in fake_client.containers._by_name
...
def test_docker_unavailable_wrapped(cfg: WebIdeConfig) -> None:
    ...
    with patch("truss_api.ide.docker_from_env", side_effect=DockerException("no sock")):
