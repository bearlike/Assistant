...
(e.g. ``@Truss``).  Non-mentioned messages are silently
...
if TYPE_CHECKING:
    from truss_core.config import AppConfig
...
def _build_help_text() -> str:
    ...
    lines = [
        "**Truss** — AI assistant with a conversation state machine and agent hypervisor",
        "",
        "**Commands** (use after @mention):",
    ]
    for name, (_, desc) in _COMMANDS.items():
        lines.append(f"- `/{name}` — {desc}")
    lines += ["", "**Usage:** `@Truss <your question or task>`"]
    return "\n".join(lines)
...
def _is_mentioned(message: InboundMessage) -> bool:
    ...
    but requires ``@Truss`` in multi-party threads.
    """
...
def init_channels(
    ...
    if nc_cfg.get("enabled") and nc_cfg.get("bot_secret"):
        adapter = NextcloudTalkAdapter(
            ...
            trigger_keyword=nc_cfg.get("trigger_keyword", "@Truss"),
        )
    ...
    if email_cfg.get("enabled"):
        from truss_api.channels.email_adapter import EmailAdapter, EmailPoller
