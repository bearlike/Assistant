...
from truss_core.classes import TaskQueue
...
from truss_core.exit_plan_mode import PLAN_DIR_ROOT, plan_file_for, session_temp_dir
from truss_core.notifications import NotificationStore
from truss_core.permissions import auto_approve
from truss_core.project_store import VirtualProject, create_project_store
...
from truss_core.tool_registry import ToolSpec, load_registry
...
# Initialize logger
logging = get_logger(name="truss-api")
logging.info("Starting Truss API server.")
...
from truss_core.hooks import HookManager as _HookManager  # noqa: E402
...
api = Api(
    ...
    version=VERSION,
    title="Truss API",
    description="Interact with Truss through a REST API",
    doc="/swagger-ui/",
    authorizations=authorizations,
    security="apikey",
)

ns = api.namespace("api", description="Truss operations")
...
if _web_ide_cfg is not None and _web_ide_cfg.enabled:
    ...
    if _mongo_db is None:
        ...
        try:
            from truss_api.ide import IdeManager, IdeStore
            from truss_api.ide_routes import ide_ns, init_ide
...
from truss_api.channels.routes import init_channels  # noqa: E402
...
class Sessions(Resource):
    ...
    def post(self) -> tuple[dict, int]:
        ...
        capabilities_header = request.headers.get("X-Truss-Capabilities", "")
...
class SessionUsage(Resource):
    ...
    def get(self, session_id: str) -> tuple[dict, int]:
        ...
        if auth_error:
            return auth_error
        from truss_core.token_budget import build_usage_numbers
...
class CommandRegistry(Resource):
    ...
    def get(self) -> tuple[dict, int]:
        ...
        if auth_error:
            return auth_error
        from truss_core.commands import list_commands
...
class SessionCommand(Resource):
    ...
    @api.doc(security="apikey")
    def post(self, session_id: str) -> tuple[dict, int]:
        """Dispatch a command, emit any required events, return the result."""
        auth_error = _require_api_key()
        if auth_error:
            return auth_error
        ...
        # Transcript-render commands also emit transcript events so chat UIs
        # show the input + result inline.
        if result.render is CommandRender.TRANSCRIPT:
            invocation = f"/{name}"
            if args:
...
class PluginMarketplace(Resource):
    ...
    def get(self) -> tuple[dict, int]:
        ...
        if auth_error:
            return auth_error
        from truss_core.config import get_config
        from truss_core.plugins import discover_marketplace_plugins
    ...
    def post(self) -> tuple[dict, int]:
        ...
        from truss_core.config import get_config
        from truss_core.plugins import install_plugin
...
class PluginDetail(Resource):
    ...
    def delete(self, plugin_name: str) -> tuple[dict, int]:
        ...
        if auth_error:
            return auth_error
        from truss_core.config import get_config
        from truss_core.plugins import uninstall_plugin
