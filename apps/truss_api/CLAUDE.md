# Truss API - Project Guidance

Scope: this file applies to the `apps/truss_api/` package. It captures runtime behavior, hidden dependencies, and testing notes so changes stay safe and predictable.

## Runtime flow (what actually happens)
- Entry point: `apps/truss_api/src/truss_api/backend.py` (HTTP API framework).
- Session endpoints:
...
- Plugins: `GET/POST /api/plugins`, `GET/POST /api/plugins/marketplace`, `DELETE /api/plugins/<name>`. Uses `truss_core.plugins` for discovery, install, uninstall. Plugin components (skills, hooks, agent definitions, MCP tools) are loaded during session init via `load_all_plugin_components()`.
...
- Orchestration: uses `truss_core.session_runtime.SessionRuntime` to run sync/async sessions. Passes `allowed_tools` from `context.mcp_tools` to scope tool binding per query.
...
- Uses core logging (`truss_core.common.get_logger`); log level controlled by `runtime.log_level`.
...
## Testing guidance
- `apps/truss_api/tests` mock `SessionRuntime.run_sync` and focus on response schema.
