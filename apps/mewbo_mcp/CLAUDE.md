...
- `create_session` — creates a session. Default: auto-provisions a fresh git worktree + branch off the target repo's base branch. Pass `branch`/`worktree` to target an existing one. `integrations` maps to `context.mcp_tools` allowlist. Wires `POST /api/v_projects/<id>/worktrees` → `POST /api/sessions` → `POST /api/sessions/<id>/query`.
- `send_followup` — `POST /api/sessions/<id>/message` (steer a running session).
- `interrupt_session` — `POST /api/sessions/<id>/interrupt`.
...
- `ask_wiki` — `POST /v1/wiki/qa` is SSE-only. Implementation: streams the start response to read the `meta` `answerId`, then polls `GET /v1/wiki/qa/<id>` until the answer is stable (block count no longer changes). Returns `{answer, citations, status}`; `status: "running"` means the timeout elapsed before completion — call again to get the rest. Completion is best-effort; the snapshot has no explicit terminal flag.

**D. Integrations / capability discovery**
- `list_integrations` — `GET /api/tools` + `GET /api/plugins`, so a caller knows what tool ids to pass to `create_session`'s `integrations` argument.
