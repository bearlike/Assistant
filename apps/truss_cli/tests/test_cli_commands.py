...
from truss_core.config import set_config_override, set_mcp_config_path  # noqa: E402
...
from truss_core.tool_registry import ToolRegistry, ToolSpec  # noqa: E402

import truss_cli.cli_commands as cli_commands
...
def test_command_compact(monkeypatch, tmp_path):
    """Compact sessions via CLI command and forward any focus directive."""
    context = _make_context(tmp_path)
    registry = get_registry()
    captured: list[str] = []

    def fake_run_sync(*args, **kwargs):
        captured.append(kwargs.get("user_query") or (args[0] if args else ""))
    ...
    assert registry.execute("/compact", context, ["focus", "on", "architecture"]) is True
    assert captured[-1] == "/compact focus on architecture"
