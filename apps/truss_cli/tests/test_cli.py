...
import json
import types
from truss_core.classes import ActionStep, Plan, PlanStep, TaskQueue, set_available_tools  # noqa: E402
from truss_core.common import get_mock_speaker  # noqa: E402
from truss_core.config import get_config_value, set_mcp_config_path  # noqa: E402
from truss_core.session_runtime import SessionRuntime  # noqa: E402
from truss_core.session_store import SessionStore  # noqa: E402
from truss_core.tool_registry import ToolRegistry, ToolSpec, load_registry  # noqa: E402
from rich.console import Console  # noqa: E402

from truss_cli.cli_commands import get_registry  # noqa: E402
from truss_cli.cli_context import CliState, CommandContext  # noqa: E402
from truss_cli.cli_master import (
...
def test_build_approval_callback_accepts(monkeypatch):
    ...
    monkeypatch.setattr(
        "truss_cli.cli_master._confirm_rich_panel", lambda *args, **kwargs: "yes"
    )
    console = Console(record=True)
...
def test_build_approval_callback_session_yes(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "truss_cli.cli_master._confirm_rich_panel", lambda *args, **kwargs: "session"
    )
...
def test_mcp_yes_always_updates_config(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "truss_cli.cli_master._confirm_rich_panel", lambda *args, **kwargs: "always"
    )
...
def test_run_query(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("truss_cli.cli_master.generate_action_plan", fake_generate)
    monkeypatch.setattr("truss_core.session_runtime.orchestrate_session", fake_orchestrate)
...
def test_run_query_auto_approves_in_headless(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("truss_cli.cli_master._build_approval_callback", fake_build)
    monkeypatch.setattr("truss_core.session_runtime.orchestrate_session", fake_orchestrate)
...
def test_run_query_dims_tool_panels_after_response(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("truss_core.session_runtime.orchestrate_session", fake_orchestrate)
    monkeypatch.setattr("truss_cli.cli_master._render_results_with_registry", fake_render)
...
def test_run_cli_single_query(monkeypatch, tmp_path):
    ...
    from truss_cli.cli_master import run_cli
    ...
    monkeypatch.setattr("truss_cli.cli_master.render_header", fake_header)
    monkeypatch.setattr("truss_core.session_runtime.orchestrate_session", fake_orchestrate)
    monkeypatch.setattr("truss_cli.cli_master.load_registry", lambda: ToolRegistry())
...
def test_run_cli_interactive_quit(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("truss_cli.cli_master.render_header", lambda *args, **kwargs: None)
    monkeypatch.setattr("truss_cli.cli_master.FileHistory", DummyHistory)
    monkeypatch.setattr(
        "truss_cli.cli_master.PromptSession",
