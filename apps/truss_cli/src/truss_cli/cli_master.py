...
from truss_core.classes import ActionStep, Plan, PlanStep, TaskQueue
from truss_core.common import MockSpeaker, format_tool_input, get_logger
from truss_core.components import resolve_langfuse_status
...
from truss_core.task_master import generate_action_plan
from truss_core.tool_registry import ToolRegistry, load_registry
...
from truss_cli.aider_ui import (
    render_diff,
    render_dir_payload,
    render_file_payload,
    render_markdown,
...
from truss_cli.cli_agent_display import AgentDisplayManager
from truss_cli.cli_commands import get_registry
from truss_cli.cli_context import CliState, CommandContext
from truss_cli.cli_dialogs import _confirm_rich_panel

logging = get_logger(name="truss.cli")
...
def _render_resume_hint(console: Console, session_id: str, session_dir: str | None) -> None:
    command = f"uv run truss --session {session_id}"
    if session_dir:
...
def run_cli(args: argparse.Namespace) -> int:
    ...
    while True:
        try:
            user_input = session.prompt("truss> ").strip()
        ...
        if user_input.startswith("/"):
            ...
            if _skill is not None and _skill.user_invocable:
                from truss_core.skills import activate_skill
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description="Truss terminal CLI")
