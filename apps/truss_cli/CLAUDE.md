# Truss CLI - UI/Terminal Guidance

Scope: this file applies to the `apps/truss_cli/` package only. It covers the terminal UI (renderer + dialog toolkit) and how CLI output is produced.
...
- Entry point: `apps/truss_cli/src/truss_cli/cli_master.py` (`run_cli`).
...
Location: `apps/truss_cli/src/truss_cli/cli_dialogs.py`
...
- Auto-fallback to plain prompt when no TTY or `TRUSS_DISABLE_TEXTUAL=1`.
...
- `apps/truss_cli/src/truss_cli/cli_master.py`: main loop, output sections, startup panel, Rich Live agent display.
- `apps/truss_cli/src/truss_cli/cli_agent_display.py`: `AgentDisplayManager` — thread-safe bridge between agent lifecycle hooks and Rich Live rendering. Handles collapsed/expanded tree, spinner, footer, elapsed time.
- `apps/truss_cli/src/truss_cli/cli_keys.py`: `KeyListener` — stdlib-only keystroke capture during Rich Live (cbreak mode + daemon thread). Reusable for future keybindings.
- `apps/truss_cli/src/truss_cli/cli_commands.py`: commands, model wizard, MCP listing.
- `apps/truss_cli/src/truss_cli/cli_dialogs.py`: dialog factory.
- `apps/truss_cli/src/truss_cli/cli_context.py`: state shared across commands.
- `packages/truss_core/src/truss_core/session_runtime.py`: shared runtime with `enqueue_message` (user steering) and `interrupt_step` (step interruption).
