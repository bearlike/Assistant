...
A terminal frontend for Truss. It runs the same orchestration loop as the API and chat UI, but in a fast, interactive shell.
...
CLI flags:
- `--config PATH` path to app config file (default: auto-discover via `CWD/configs/` → `$TRUSS_HOME/` → `~/.truss/`).
