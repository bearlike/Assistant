...
mailbox becomes a Meeseeks chat channel.  New emails start sessions;
...
* **Multi-party** threads require ``@Meeseeks`` in the body.
"""
...
from meeseeks_api.channels.base import InboundMessage
...
class EmailAdapter:
    """Channel adapter that bridges an email mailbox to Meeseeks sessions.
    ...
    def requires_mention(self, message: InboundMessage) -> bool:
        """Return True when @Meeseeks must appear in the message.
    ...
    def _build_thread_headers(
        ...
        subject = meta.subject if meta else "Meeseeks"
    ...
    def system_context(self) -> str:
        ...
        from meeseeks_api.channels.routes import _COMMANDS
