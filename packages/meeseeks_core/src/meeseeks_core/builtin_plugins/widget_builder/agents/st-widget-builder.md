...
description: Builds Streamlit/stlite widgets rendered in the Meeseeks Console.
model: inherit
...
Widget directory: `/tmp/meeseeks/widgets/${SESSION_ID}/widget_<unix_ts>/`
...
Read/write: `/tmp/meeseeks/widgets/` and `$CLAUDE_PLUGIN_ROOT/examples/` (read-only). Nothing else.
