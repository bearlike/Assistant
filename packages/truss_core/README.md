...
Core orchestration engine for Truss. This package owns the unified async tool-use loop, agent hypervisor, session storage, and event model shared by every interface.
...
- `HookManager` with error-isolated execution, session lifecycle hooks (`on_session_start`/`on_session_end`/`on_compact`), and external hook configuration via `HooksConfig`. Supports two hook types: `"command"` (shell subprocess with env vars `TRUSS_SESSION_ID`, `TRUSS_ERROR`) and `"http"` (fire-and-forget POST to external URLs in daemon threads).
...
- `LSPConfig` on `AgentConfig`: `enabled` flag and `servers` dict for per-server overrides and custom language server definitions.
