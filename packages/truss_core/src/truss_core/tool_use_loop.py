...
from truss_core.common import get_git_context, get_logger, get_mock_speaker, get_system_prompt
from truss_core.components import (
    build_langfuse_handler,
    langfuse_propagate,
    langfuse_trace_span,
)
from truss_core.config import get_config_value, get_version
from truss_core.context import ContextSnapshot, render_event_lines
from truss_core.exit_plan_mode import (
    SHELL_TOOL_IDS,
    ExitPlanModeTool,
    ensure_plan_dir,
    is_inside_plan_dir,
...
from truss_core.llm import build_chat_model, specs_to_langchain_tools
from truss_core.permissions import PermissionDecision, PermissionPolicy
from truss_core.session_tools import (
...
class ToolUseLoop:
        ...
        *,
        ...
        Args:
            ...
            session_capabilities: Client-advertised capability tuple from the
                ``X-Truss-Capabilities`` header (persisted on the session
        ...
        if agent_context.can_spawn:
            from truss_core.spawn_agent import SpawnAgentTool
        ...
        *,
        ...
        try:
            ...
            langfuse_handler = build_langfuse_handler(
                user_id="truss-tool-use",
                session_id=f"tool-use-{self._ctx.agent_id}",
                trace_name="truss-tool-use",
                version=get_version(),
    ...
    def _configured_edit_tool_id(self) -> str:
        ...
        """
        from truss_core.llm import model_prefers_structured_patch
    ...
    def _should_compact_messages(self, messages: list[BaseMessage]) -> bool:
        ...
        from truss_core.token_budget import get_model_max_input_tokens
    ...
    def _bind_model(self, tool_schemas: list[dict[str, Any]]) -> Any:
        ...
        if (not plan_mode or plan_root) and self._spawn_agent_tool is not None:
            from truss_core.spawn_agent import SPAWN_AGENT_SCHEMA
            ...
            if self._ctx.depth == 0:
                from truss_core.spawn_agent import (
                    CHECK_AGENTS_SCHEMA,
                    STEER_AGENT_SCHEMA,
                )
        ...
        if (
            ...
            from truss_core.skills import ACTIVATE_SKILL_SCHEMA
...
def _append_lsp_feedback(content: str, file_path: str, cwd: str) -> str:
    ...
    try:
        from truss_tools.integration.lsp import get_passive_diagnostics
