#!/usr/bin/env python3
"""Core data models and tool abstractions for Truss orchestration."""
...
from truss_core.common import MockSpeaker, get_logger, get_mock_speaker, get_unique_timestamp
from truss_core.components import build_langfuse_handler
from truss_core.config import get_config_value, get_version
from truss_core.llm import build_chat_model
from truss_core.types import ActionStepPayload, ToolInput
...
class AbstractTool(abc.ABC):
    ...
    def __init__(
        ...
        self.langfuse_handler = build_langfuse_handler(
            user_id=f"truss-{name}",
            session_id=session_id,
            trace_name=f"truss-{self._id}",
            version=get_version(),
