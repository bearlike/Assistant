#!/usr/bin/env python3
"""Central JSON configuration for Truss."""
...
_PACKAGE_NAMES = ("truss-core", "truss-workspace")


def resolve_truss_home() -> Path:
    """Return the Truss home directory (``$TRUSS_HOME`` or ``~/.truss``)."""
    env = os.environ.get("TRUSS_HOME")
    if env:
        return Path(env).expanduser().resolve()
    return Path.home() / ".truss"


def _resolve_config_path(filename: str) -> Path:
    """Find a config file: ``CWD/configs/`` first, then ``TRUSS_HOME``."""
    cwd_path = Path("configs") / filename
    if cwd_path.exists():
        return cwd_path
    return resolve_truss_home() / filename


def get_version() -> str:
    """Return the package version from pyproject.toml (via importlib.metadata).

    Tries ``truss-core`` first (always installed), then the workspace
...
class RuntimeConfig(BaseModel):
    ...
    cache_dir: str = Field(
        "",
        description="Directory for tool caches. Defaults to $TRUSS_HOME/cache.",
        examples=["~/.truss/cache"],
        json_schema_extra={"x-protected": True},
    )
    session_dir: str = Field(
        "",
        description="Directory for session transcripts. Defaults to $TRUSS_HOME/sessions.",
        examples=["~/.truss/sessions"],
        json_schema_extra={"x-protected": True},
    )
    config_dir: str = Field(
        "",
        description="Root configuration directory. Defaults to $TRUSS_HOME.",
        examples=["~/.truss"],
        json_schema_extra={"x-protected": True},
    )
    result_export_dir: str = Field(
        ...
        examples=["/tmp/truss-results"],
    )
    projects_home: str = Field(
        "",
        description="Directory for virtual project folders. Defaults to $TRUSS_HOME/projects.",
        examples=["~/.truss/projects"],
        json_schema_extra={"x-protected": True},
    )
    ...
    def _normalize_projects_home(cls, value: Any) -> str:
        ...
        return str(resolve_truss_home() / "projects")
...
class PluginsConfig(BaseModel):
    ...
    install_path: str = Field(
        "",
        description=(
            "Override install path for Truss-managed plugins. "
            "Defaults to $TRUSS_HOME/plugins/ (via resolve_truss_home)."
        ),
    )
    ...
    def resolve_install_dir(self) -> Path:
        """Uses install_path if set, otherwise resolve_truss_home() / 'plugins'."""
        if self.install_path:
            return Path(self.install_path)
        return resolve_truss_home() / "plugins"
    ...
    def resolve_marketplace_dirs(self, *, sync: bool = True) -> list[Path]:
        ...
        if sync and not dirs and self.marketplaces:
            from truss_core.plugins import sync_marketplaces
...
class WebIdeConfig(BaseModel):
    ...
    network: str = Field(default="truss-ide", pattern=r"^[a-zA-Z0-9_-]+$")
    state_dir: str = Field(default="/tmp/truss-ide")
...
class MongoDBConfig(BaseModel):
    ...
    uri: str = Field(
        ...
        examples=["mongodb://user:pass@localhost:27017/truss?authSource=admin"],
    )
    database: str = Field(
        "truss",
    ...
    def _normalize_uri(cls, value: Any) -> str:
        env = os.environ.get("TRUSS_MONGODB_URI")
        if env:
            return env
    ...
    def _normalize_database(cls, value: Any) -> str:
        env = os.environ.get("TRUSS_MONGODB_DATABASE")
        if env:
            return env
        return str(value).strip() if value else "truss"
...
class StorageConfig(BaseModel):
    ...
    def _normalize_driver(cls, value: Any) -> str:
        env = os.environ.get("TRUSS_STORAGE_DRIVER")
        if env:
            value = env
...
class AppConfig(BaseModel):
    """Typed configuration for the Truss runtime."""
...
def _example_app_payload() -> dict[str, Any]:
    ...
    # Reset resolved paths to empty so users get $TRUSS_HOME defaults
...
def _default_example_path(filename: str) -> Path:
    """Return the example config path: ``CWD/configs/`` if present, else ``TRUSS_HOME``."""
