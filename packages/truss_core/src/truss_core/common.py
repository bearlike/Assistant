...
def get_system_prompt(name: str = "action-planner") -> str:
    ...
    prompt_resource = resources.files("truss_core").joinpath("prompts").joinpath(f"{name}.txt")
...
def discover_subtree_instructions(
    ...
    *,
    ...
    Respects the ``<!-- truss:noload -->`` marker (checked via first line).
    """
    work_dir = Path(cwd) if cwd else Path.cwd()
...
def discover_project_instructions(cwd: str | None = None) -> str | None:
    ...
    Files containing ``<!-- truss:noload -->`` on the first line are skipped.
...
def render_jinja_prompt(name: str, **variables: object) -> str:
    """Render a Jinja2 prompt template from ``truss_core/prompts/``.
    ...
    template_env = Environment(loader=PackageLoader("truss_core", "prompts"))
