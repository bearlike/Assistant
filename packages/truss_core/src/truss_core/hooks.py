...
def _hook_env(action_step: ActionStep, result_content: str | None = None) -> dict[str, str]:
    ...
    env = dict(os.environ)
    env["TRUSS_TOOL_ID"] = action_step.tool_id or ""
    env["TRUSS_OPERATION"] = action_step.operation or ""
    if result_content is not None:
        env["TRUSS_TOOL_RESULT"] = result_content[:2000]
    return env
...
def _session_env(session_id: str, error: str | None = None) -> dict[str, str]:
    ...
    env = dict(os.environ)
    env["TRUSS_SESSION_ID"] = session_id
    if error is not None:
        env["TRUSS_ERROR"] = error
    return env
...
def merge_plugin_hooks(
    ...
    """
    from truss_core.config import HookEntry
    from truss_core.plugins import substitute_plugin_vars
