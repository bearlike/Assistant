...
from truss_core.session_store import SessionStoreBase, _utc_now
from truss_core.types import Event, EventRecord
...
class MongoSessionStore(SessionStoreBase):
    ...
    def __init__(
        ...
        *,
        ...
        if database is None:
            database = get_config_value("storage", "mongodb", "database", default="truss")
        ...
        try:
            ...
            raise ConnectionError(
                ...
                f"Check TRUSS_MONGODB_URI and ensure MongoDB is running. "
                f"Error: {exc}"
            ) from exc
