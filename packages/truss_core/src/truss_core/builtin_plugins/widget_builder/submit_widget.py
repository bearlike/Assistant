...
from truss_core.common import (
    MockSpeaker,
    get_logger,
    pydantic_to_openai_tool,
)
from truss_core.session_tools import DEFAULT_SESSION_TOOL_MODES
...
_DEFAULT_WIDGET_ROOT = "/tmp/truss/widgets"


def _widget_root() -> str:
    """Return the base widget directory.

    Reads ``TRUSS_WIDGET_ROOT`` from the environment and falls back to
    ``/tmp/truss/widgets``. The agent prompt uses the same
    ``${TRUSS_WIDGET_ROOT:-/tmp/truss/widgets}`` pattern so writer
    ...
    """
    return os.environ.get("TRUSS_WIDGET_ROOT") or _DEFAULT_WIDGET_ROOT
...
class SubmitWidgetTool:
    ...
    ``${TRUSS_WIDGET_ROOT}/{session_id}/{widget_id}/`` (default
    ``/tmp/truss/widgets``), emits a ``widget_ready`` event, and sets a
