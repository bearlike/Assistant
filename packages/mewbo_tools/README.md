...
Tool implementations and integrations for Mewbo. This package ships the built‑in local tools (file read/list/edit, shell) plus integrations for Home Assistant and MCP.
...
- **Native LSP code intelligence:** `lsp_tool` backed by pygls/lsprotocol. Operations: `diagnostics`, `definition`, `references`, `hover`. Built-in server definitions for pyright (Python), typescript-language-server (TS/JS), gopls (Go), and rust-analyzer (Rust) — auto-discovered via `shutil.which`, spawned lazily per-session. Passive diagnostics inject automatically after file edits via a hook in `ToolUseLoop`. Configure via `agent.lsp` in config. Implementation: `integration/lsp/` (`__init__.py`, `manager.py`, `servers.py`, `tool.py`).
