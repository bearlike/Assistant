...
plan to ``/tmp/mewbo/plans/<session_id>/plan.md`` using the configured edit
...
PLAN_DIR_ROOT = "/tmp/mewbo/plans"
"""Root directory for all per-session plan scratch directories."""

SESSION_TEMP_ROOT = "/tmp/mewbo/sessions"
...
def plan_dir_for(session_id: str) -> str:
    ...
    Returns:
        Absolute path ``/tmp/mewbo/plans/<session_id>/``.
    """
...
class ExitPlanModeTool:
    ...
    the :class:`~mewbo_core.session_tools.SessionTool` Protocol.
    """
