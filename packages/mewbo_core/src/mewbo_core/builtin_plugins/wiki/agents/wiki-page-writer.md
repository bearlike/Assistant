...
model: inherit
tools: [read_file, glob, grep, code_search, wiki_query_graph, wiki_submit_page]
...
2. **Gather additional context** — use `grep`, `glob`, `code_search`, or `wiki_query_graph` to find cross-references, callers, or related symbols not in `relevantFiles`. Keep to what the page directly covers.
