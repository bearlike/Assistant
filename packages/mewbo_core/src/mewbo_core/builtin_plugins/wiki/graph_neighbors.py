...
logging = get_logger(name="core.builtin_plugins.wiki.graph_neighbors")
...
class WikiGraphNeighbors:
    ...
    @classmethod
    def for_session(cls, session_id: str) -> WikiGraphNeighbors | dict[str, Any]:
        """Resolve indexer or QA ctx + build a neighbours view.

        Returns a fresh instance on success, or a wire-ready error
        dict (matching ``_err_result``'s shape) the caller can hand
        back to the LLM.
        """
...
class WikiGraphNeighborsTool:
    ...
    async def handle(self, action_step: ActionStep) -> MockSpeaker:
        ...
        if isinstance(view, dict):
            return MockSpeaker(content=str(view))
        try:
            args = WikiGraphNeighborsArgs.model_validate(action_step.tool_input or {})
