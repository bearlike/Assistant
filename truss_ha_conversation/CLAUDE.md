# Truss HA Conversation - Project Guidance

Scope: this file applies to `truss_ha_conversation/` (home automation integration).

## Runtime flow
- Entry point for API calls: `truss_ha_conversation/api.py`.
- Uses an async HTTP client + timeout helper to call `POST /api/query` on the Truss API.
- Returns a parsed `TrussQueryResponse` to the home automation host.

## Hidden dependencies / assumptions
- Uses a hardcoded API key in `TrussApiClient` (`msk-strong-password`). Override for real deployments.
