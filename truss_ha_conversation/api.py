...
class TrussQueryResponse(TypedDict):
...
class TrussApiClient:
    ...
    def __init__(
        ...
        Args:
            base_url: Base URL for the Truss API.
    ...
    async def async_generate(self, data: dict[str, Any] | None = None) -> TrussQueryResponse:
        ...
        result = await self._truss_api_wrapper(
            method="post",
            url=url_query,
            data=data_custom,
            headers=None,
        ...
        if isinstance(result, str):
            raise ApiJsonError("Unexpected text response from Truss API.")
        return result

    async def _truss_api_wrapper(
        self,
        method: str,
        url: str,
    ...
    ) -> TrussQueryResponse | str:
        """Perform an HTTP request to the Truss API.
        ...
        async with async_timeout.timeout(self.timeout):
            ...
            if decode_json:
                ...
                response_data: TrussQueryResponse = {
