"""Custom integration to integrate truss_conversation with Home Assistant.
...
class TrussMessage(TypedDict, total=False):
    """Message structure used to call the Truss API."""
...
class TrussResponse(TypedDict):
    """Response payload returned from the Truss API."""
...
try:
    ...
    from .api import TrussApiClient
    from .const import CONF_BASE_URL, CONF_TIMEOUT, DEFAULT_TIMEOUT
    from .coordinator import TrussDataUpdateCoordinator
...
def _missing_homeassistant_error() -> RuntimeError:
    error = RuntimeError(
        "Home Assistant is not installed. Install truss-ha-conversation[homeassistant]."
    )
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    """Set up Truss conversation using UI.
    ...
    Raises:
        ConfigEntryNotReady: If the Truss API is unreachable.
    """
    ...
    client = TrussApiClient(
        base_url=entry.data[CONF_BASE_URL],
        timeout=entry.options.get(CONF_TIMEOUT, DEFAULT_TIMEOUT),
        session=async_get_clientsession(hass),
    )

    hass.data[DOMAIN][entry.entry_id] = coordinator = TrussDataUpdateCoordinator(
        hass,
        client,
    )
    ...
    try:
        ...
        if not response:
            raise ApiClientError("Invalid Truss server")
    except ApiClientError as err:
        raise ConfigEntryNotReady(err) from err

    entry.async_on_unload(entry.add_update_listener(async_reload_entry))

    conversation.async_set_agent(hass, entry, TrussAgent(hass, entry, client))
    return True


async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    """Unload Truss conversation.
...
class TrussAgent(_BaseConversationAgent):
    """Truss conversation agent."""
    ...
    def __init__(
        ...
        self.entry = entry
        self.client = client
        self.history: dict[str, TrussMessage] = {}
