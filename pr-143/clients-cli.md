...
<video controls preload="metadata" poster="../truss-console-01-front.png" style="width: 100%; max-width: 960px; height: auto; display: block; margin: 0 auto;">
  <source src="../truss-cli-01-video.mp4" type="video/mp4" />
...
```bash
truss --tag my-project       # create or resume a tagged session
truss --session <session-id> # resume by ID
```
...
```bash
truss --fork my-project      # fork from a tagged session at its current state
```
