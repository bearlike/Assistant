...
Truss runs as a local CLI, a REST API + web console, or a containerised stack. Pick the path that fits your setup.
...
For a global install so `truss` works from anywhere:
```bash
uv tool install .
```
...
cd apps/truss_console && npm install && cd -

# Start the API
uv run truss-api

# Start the console (separate terminal)
cd apps/truss_console && npm run dev
```
...
Truss loads config files from the first directory that contains them, checked in order:

1. `CWD/configs/`: project-local config (highest priority)
2. `$TRUSS_HOME/`: custom home dir if set
3. `~/.truss/`: user home fallback
...
Truss discovers `CLAUDE.md`, `AGENTS.md`, and `.claude/rules/*.md` files
