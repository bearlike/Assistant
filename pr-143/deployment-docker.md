...
The recommended way to run Truss in a persistent environment. Pre-built images for the API, console, and base layer are published to GHCR. A single `docker compose up` starts the full stack. It includes the API, console, MongoDB, and the nginx proxy for the Web IDE feature.
...
| `MONGO_INITDB_ROOT_USERNAME` | No | `truss` | MongoDB root username. |
| `MONGO_INITDB_ROOT_PASSWORD` | No | `truss` | MongoDB root password. Change this in production. |
...
|---------|-------|-------------|---------|
| `api` | `ghcr.io/bearlike/truss-api` | `5125` | Gunicorn + Flask REST API. Runs as `HOST_UID:HOST_GID`. |
| `console` | `ghcr.io/bearlike/truss-console` | `3001` | nginx serving the React SPA. Proxies `/api/` to the API. |
...
Both `api` and `console` use **host networking** (`network_mode: host`), so they share `127.0.0.1` with the host. The `ide-proxy` runs on the `truss-ide` bridge network and is bound to loopback by default.
...
| `plans-data` | `/tmp/truss/plans` | Plan-mode scratch files (survive restarts). |
...
| Image | Purpose |
|-------|---------|
| `ghcr.io/bearlike/truss-api` | REST API (Gunicorn + Flask). |
| `ghcr.io/bearlike/truss-console` | Web console (nginx + React SPA). |
| `ghcr.io/bearlike/truss-base` | Base image with Python, Node, and core packages. Used as build arg for the API image. |
