#!/usr/bin/env python3
"""Unit tests for ``meeseeks_core.session_tools``.
...
from meeseeks_core.session_tools import SessionToolFactory, SessionToolRegistry
...
class TestLoadEntry:
    ...
    def test_load_entry_valid_record_registers_factory(self):
        module_name = "meeseeks_test_session_tools_fixture"
    ...
    def test_load_entry_missing_class_is_skipped(self):
        reg = SessionToolRegistry()
        reg.load_entry(
            {"tool_id": "fake_tool", "module": "meeseeks_core.session_tools"}
        )
        assert reg.build_for(["fake_tool"], session_id="s1", event_logger=None) == []
    ...
    def test_load_entry_missing_class_attr_is_skipped(self):
        module_name = "meeseeks_test_session_tools_attr_fixture"
        self._make_fixture_module(module_name, _FakeSessionTool)
        try:
            reg = SessionToolRegistry()
            reg.load_entry(
    ...
    def test_load_entry_first_wins_no_override(self):
        ...
        module_a = "meeseeks_test_session_tools_first"
        module_b = "meeseeks_test_session_tools_second"
