...
``meeseeks_core/builtin_plugins/widget_builder/``. Keeping them integration-
...
def _plugin_root() -> Path:
    traversable = importlib.resources.files("meeseeks_core") / "builtin_plugins" / "widget_builder"
...
class TestManifest:
    def test_manifest_parses_with_stlite_capability(self):
        from meeseeks_core.plugins import parse_plugin_manifest
...
class TestAgentAndSkill:
    def test_skill_parses_with_agent_and_capability(self):
        ...
        from meeseeks_core.skills import _parse_skill_file
    ...
    def test_agent_parses_with_submit_widget_and_capability(self):
        from meeseeks_core.agent_registry import parse_agent_file
...
class TestSessionToolLoad:
    ...
    def test_submit_widget_tool_direct_construction(self):
        from meeseeks_core.builtin_plugins.widget_builder.submit_widget import (
            SubmitWidgetTool,
        )
...
def test_handle_runtime_traversal_guard_rejects_symlink_escape(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("MEESEEKS_WIDGET_ROOT", str(widget_root))
    tool = SubmitWidgetTool(session_id="s1", event_logger=None)
