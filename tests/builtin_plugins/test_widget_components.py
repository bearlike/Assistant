...
def _components_dir() -> Path:
    return Path(
        str(
            importlib.resources.files("meeseeks_core")
            / "builtin_plugins"
            / "widget_builder"
            / "examples"
            / "components"
...
class TestComponentFile:
    def test_lints_clean_against_widget_allowlist(self, component_file: Path):
        """Every component must pass the same AST linter ``submit_widget`` runs.

        If this test fails, a future widget that copies the component will
        be rejected at submit time — the "copy-paste and ship" promise breaks.
        """
        findings = lint(component_file.read_text())
        assert not findings, (
            f"{component_file.name} would fail the widget AST linter:\n"
