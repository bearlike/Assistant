...
# -- MEWBO_HOME resolution chain ------------------------------------------


class TestResolveMewboHome:
    """Tests for resolve_mewbo_home()."""

    def test_env_var_takes_precedence(self, monkeypatch, tmp_path):
        monkeypatch.setenv("MEWBO_HOME", str(tmp_path / "custom"))
        result = config_module.resolve_mewbo_home()
        assert result == (tmp_path / "custom").resolve()

    def test_default_is_home_dot_mewbo(self, monkeypatch):
        monkeypatch.delenv("MEWBO_HOME", raising=False)
        from pathlib import Path

        result = config_module.resolve_mewbo_home()
        assert result == Path.home() / ".mewbo"

    def test_tilde_in_env_expanded(self, monkeypatch):
        monkeypatch.setenv("MEWBO_HOME", "~/my-mewbo")
        result = config_module.resolve_mewbo_home()
        assert "~" not in str(result)
        assert "my-mewbo" in str(result)
...
class TestResolveConfigPath:
    ...
    def test_home_file_found(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("MEWBO_HOME", str(home))
        result = config_module._resolve_config_path("app.json")
...
class TestGetConfigPathIntegration:
    ...
    def test_app_config_falls_back_to_home(self, monkeypatch, tmp_path):
        monkeypatch.chdir(tmp_path)
        monkeypatch.setenv("MEWBO_HOME", str(tmp_path / "mhome"))
        config_module.reset_config()
        path = config_module.get_app_config_path()
...
class TestEnsureExampleConfigs:
    """ensure_example_configs respects MEWBO_HOME when outside project."""
