...
class TestCompactConversationFocus:
    def test_focus_prompt_threaded_into_system_message(self):
        ...
        with patch("truss_core.llm.build_chat_model", return_value=llm):
            asyncio.run(
                compact_conversation(
                    [_user("hi")],
...
class TestCompactConversation:
    ...
    def test_partial_keeps_recent(self):
        ...
        with (
            patch("truss_core.llm.build_chat_model", return_value=llm),
            patch("truss_core.compact.get_config_value", return_value=2),
        ):
            r = asyncio.run(
                compact_conversation(
                    [
    ...
    def test_partial_nothing_to_summarize(self):
        with patch("truss_core.compact.get_config_value", return_value=10):
            r = asyncio.run(
                compact_conversation(
                    [_user("recent")],
                    mode=CompactionMode.PARTIAL,
...
class TestGetCompactPrompt:
    ...
    def test_defaults_to_standard_when_config_missing(self):
        ...
        with patch("truss_core.compact.get_config_value", return_value=False):
            assert get_compact_prompt() is COMPACT_PROMPT

    def test_caveman_mode_true_returns_caveman_prompt(self):
        with patch("truss_core.compact.get_config_value", return_value=True):
...
class TestResolveCompactModels:
    ...
    def test_non_list_config_falls_back(self):
        ...
        with patch("truss_core.compact.get_config_value", return_value=42):
            assert resolve_compact_models("sonnet") == ["sonnet"]
...
class TestCompactModelFallback:
    def test_first_model_fails_second_succeeds(self):
        ...
        with (
            patch("truss_core.llm.build_chat_model", side_effect=_build),
            patch(
                "truss_core.compact.resolve_compact_models",
