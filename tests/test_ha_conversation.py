...
import truss_ha_conversation as ha_module  # noqa: E402
from truss_ha_conversation import (  # noqa: E402
    TrussAgent,
)
from truss_ha_conversation.api import TrussApiClient  # noqa: E402
from truss_ha_conversation.config_flow import (  # noqa: E402
    TrussConfigFlow,
    TrussOptionsFlow,
)
from truss_ha_conversation.coordinator import (  # noqa: E402
    TrussDataUpdateCoordinator,
)
from truss_ha_conversation.exceptions import ApiClientError  # noqa: E402
from truss_ha_conversation.helpers import get_exposed_entities  # noqa: E402
...
def test_api_generate_includes_session_id():
    ...
    client = TrussApiClient(base_url="http://test", timeout=10, session=session)
...
def test_coordinator_update_success():
    ...
    coordinator = TrussDataUpdateCoordinator(hass=None, client=Client())
...
def test_agent_process_success():
    ...
    agent = TrussAgent(hass, entry, client)
...
def test_config_flow_success(monkeypatch):
    ...
    monkeypatch.setattr(TrussApiClient, "async_get_heartbeat", fake_heartbeat)
    user_input = {
        "base_url": "http://test",
        "api_key": "token",
        "timeout": 10,
...
def test_options_flow_menu():
    ...
    flow = TrussOptionsFlow(types.SimpleNamespace(options={}))
...
def test_async_setup_and_unload_entry(monkeypatch):
    ...
    from truss_ha_conversation.const import CONF_BASE_URL, CONF_TIMEOUT
    ...
    monkeypatch.setattr(ha_module, "TrussApiClient", DummyClient)
    monkeypatch.setattr(ha_module, "TrussDataUpdateCoordinator", DummyCoordinator)
...
def test_api_wrapper_decode_json_false():
    ...
    client = TrussApiClient(base_url="http://test", timeout=10, session=Session())
    result = asyncio.run(client._truss_api_wrapper("get", "http://test", decode_json=False))
    assert result == "plain"
...
def test_api_wrapper_handles_404():
    ...
    client = TrussApiClient(base_url="http://test", timeout=10, session=Session())
    try:
        asyncio.run(client._truss_api_wrapper("get", "http://test"))
    except Exception as exc:
