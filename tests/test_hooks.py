...
from truss_core.permissions import PermissionDecision
...
class TestCommandHookPassesEnv:
    ...
    def test_pre_tool_hook_passes_env(self):
        ...
        with patch("truss_core.hooks.subprocess.run") as mock_run:
            hook(step)
            mock_run.assert_called_once()
            call_kwargs = mock_run.call_args
            passed_env = call_kwargs.kwargs.get("env") or call_kwargs[1].get("env")
            assert passed_env["TRUSS_TOOL_ID"] == "my_tool"
            assert passed_env["TRUSS_OPERATION"] == "set"
    ...
    def test_post_tool_hook_passes_result_env(self):
        ...
        with patch("truss_core.hooks.subprocess.run") as mock_run:
            ...
            call_kwargs = mock_run.call_args
            passed_env = call_kwargs.kwargs.get("env") or call_kwargs[1].get("env")
            assert passed_env["TRUSS_TOOL_ID"] == "shell_tool"
            assert passed_env["TRUSS_TOOL_RESULT"] == "output data"
