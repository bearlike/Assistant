...
import pytest
import truss_core.classes as classes
from truss_core.classes import ActionStep, TaskQueue, create_task_queue, set_available_tools
...
def test_save_json(tmp_path, monkeypatch):
    ...
    from truss_core.classes import AbstractTool
...
def test_abstract_tool_cache_dir_empty_resolves_home(monkeypatch):
    """Empty cache_dir resolves to TRUSS_HOME/cache/<tool_id>."""
    ...
    tool = DummyTool()
    # Empty cache_dir is resolved by RuntimeConfig to ~/.truss/cache
    assert ".truss" in tool.cache_dir or "cache" in tool.cache_dir
