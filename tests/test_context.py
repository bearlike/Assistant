...
from truss_core.context import ContextBuilder, event_payload_text, render_event_lines
...
def test_select_context_events_keep_ids(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "truss_core.context.ChatPromptTemplate", lambda *args, **kwargs: DummyPrompt(selection)
    )
    monkeypatch.setattr("truss_core.context.build_chat_model", lambda **_k: object())
    builder = ContextBuilder(SessionStore(root_dir=str(tmp_path)))
    events = [
...
def test_recovery_continue_rendered_context_contains_original_task(tmp_path):
    ...
    original_task = "restructure the Truss landing page layout"
