...
from truss_core.orchestrator import Orchestrator
...
def _patch_plan_config(
    *,
    ...
    """
    from truss_core import tool_use_loop as _tul
    ...
    return patch("truss_core.tool_use_loop.get_config_value", side_effect=fake_get)
...
class TestPathHelpers:
    def test_plan_dir_for_is_session_scoped(self):
        assert plan_dir_for("abc123").endswith("/tmp/truss/plans/abc123")
...
class TestAllowedRootsIncludesPlanDir:
    ...
    Plan mode sends edits to ``/tmp/truss/plans/<session_id>/plan.md``.
    The upper guard (``_plan_mode_permission``) approves such writes via
    ``is_inside_plan_dir``. The lower guard (``resolve_safe_path`` in
    ``truss_tools.core``) must also accept them — otherwise edits are
    ...
    def test_get_allowed_roots_includes_plan_dir_root(self):
        ...
        from truss_tools.core import _get_allowed_roots
    ...
    def test_resolve_safe_path_accepts_truss_scratch_paths(self):
        """``/tmp/truss/...`` is allowed — covers widget output and plan files."""
        from truss_tools.core import resolve_safe_path

        resolved = resolve_safe_path("/tmp/truss/widgets/abc/app.py")
        assert str(resolved).startswith("/tmp/truss/")

    def test_resolve_safe_path_rejects_unrelated_tmp_paths(self):
        """Arbitrary ``/tmp`` files are NOT allowed — only ``/tmp/truss``.
        ...
        try:
            resolve_safe_path("/tmp/not-a-truss-file.sh")
...
class TestEpisodicPlanApproval:
    ...
    def _make_runtime(self):
        ...
        from truss_core.session_store import create_session_store
...
class TestEditToolWritesPlanMdEndToEnd:
    ...
    def test_file_edit_tool_writes_plan_md(self):
        from truss_core.classes import ActionStep as AS
        from truss_tools.integration.file_edit_tool import FileEditTool
