...
- **CLI integration** (`apps/truss_cli/tests/test_cli.py`): full CLI flows using lightweight stubs.
- **HTTP hooks** (`test_hooks_http.py`): HTTP hook factories (pre/post tool, session start/end), matcher filtering, mixed command+http hooks, session env var enrichment (`TRUSS_SESSION_ID`, `TRUSS_ERROR`). Mocks `_post_json` to avoid real HTTP.
