...
A multi-agent LLM personal assistant. Core engine: a single async `ToolUseLoop` that the LLM drives via native `bind_tools`. Sub-agents are spawned via `spawn_agent`, tracked by `AgentHypervisor`, cleaned up via structured concurrency. Interfaces: CLI, web console, REST API, Home Assistant, Nextcloud Talk — all share the core engine.
